/**
 * SEO Metadata for Cryopreservation Services
 * Auto-generated for canonical URLs and structured data
 */

import { Metadata } from 'next';

export const metadata: Metadata = {
  title: 'Cryopreservation Services | ingenious targeting laboratory',
  description: 'Cryopreservation Services from ingenious targeting laboratory. Custom mouse model engineering since 1998.',
  alternates: {
    canonical: 'https://www.genetargeting.com/cryopreservation-services',
  },
  openGraph: {
    title: 'Cryopreservation Services | ingenious targeting laboratory',
    description: 'Cryopreservation Services from ingenious targeting laboratory. Custom mouse model engineering since 1998.',
    url: 'https://www.genetargeting.com/cryopreservation-services',
    siteName: 'ingenious targeting laboratory',
    locale: 'en_US',
    type: 'website',
  },
  robots: {
    index: true,
    follow: true,
  },
};

// BreadcrumbList structured data
export const breadcrumbSchema = {
  '@context': 'https://schema.org',
  '@type': 'BreadcrumbList',
  itemListElement: [
    { "@type": "ListItem", position: 1, name: "Home", item: "https://www.genetargeting.com" },
    { "@type": "ListItem", position: 2, name: "Services", item: "https://www.genetargeting.com/mouse-model-services" },
    { "@type": "ListItem", position: 3, name: "Cryopreservation Services", item: "https://www.genetargeting.com/cryopreservation-services" }
  ],
};
