/**
 * Lineage Tracing Mouse Models Page
...
// FAQ Data
const faqData = [
  { question: "What reporter systems are used for lineage tracing?", answer: "Common lineage tracing reporters include Rosa26-tdTomato (red fluorescent protein), Rosa26-GFP (green fluorescent protein), Rosa26-LacZ (beta-galactosidase), and multi-color systems like Rosa26-Confetti (stochastic color switching). Selection depends on imaging modality, tissue type, and whether multi-color clonal analysis is needed." },
  { question: "How does CreER enable temporal lineage tracing?", answer: "Tamoxifen-inducible Cre (CreER) provides temporal control of reporter activation. A single tamoxifen pulse marks cells expressing the Cre driver at that moment. All descendants of those marked cells retain the reporter, enabling tracking of cell fate decisions and clonal expansion over time." },
  { question: "Can lineage tracing be combined with conditional knockout?", answer: "Yes. Lineage tracing and conditional knockout can be combined by crossing a CreER driver to both a reporter line and a floxed target gene. Tamoxifen treatment simultaneously marks cells (lineage tracing) and deletes the target gene (conditional knockout), enabling study of gene function specifically in traced cell populations." },
  { question: "What Cre drivers are commonly used for lineage tracing?", answer: "Common CreER drivers include tissue specific promoters (e.g., Myh6-CreER for cardiomyocytes, Albumin-CreER for hepatocytes, Nestin-CreER for neural cells) and cell type specific promoters (e.g., alpha-SMA-CreER for smooth muscle, CD4-CreER for T cells). Selection depends on the cell population you want to trace." }
];

// Related Links
const relatedLinks: Array<{ title: string; href: string }> = [];

export default function LineageTracingMouseModelsPage() {
      ...
      <main id="main-content">
        ...
        <section style={{ backgroundColor: '#f8f9fa', padding: '50px 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <LabSignalsSignup 
              variant="card"
              title="Lineage Tracing Research Updates"
              description="Discover reporter systems, Cre drivers, and cell fate mapping strategies from expert PhD scientists."
            />
          </div>
        </section>
          ...
          __html: JSON.stringify({
            ...
            "itemListElement": [
              ...
              },
              {
                "@type": "ListItem",
                "position": 2,
                "name": "Lineage Tracing Mouse Models",
                "item": "https://www.genetargeting.com/lineage-tracing-mouse-models"
              }
            ]
          })
        }}
