---
title: "Custom Projects"
slug: "custom-projects"
extracted: "2026-01-09T17:03:04.058Z"
---

# Custom Mouse Model Projects | Knockout, Knockin, Humanized | Ingenious Targeting Laboratory

**Url: /custom-projects**

**Meta Description: Custom mouse model generation tailored to your research. Knockout, knockin, conditional, and humanized models. 2,500+ projects since 1998.**

**Primary Keyword: custom mouse model projects**

**Secondary Keywords: custom knockout mouse, custom knockin, tailored mouse models, bespoke gene targeting**

**Word Count: 1800**

# Custom Mouse Model Projects

Every research question is unique. Ingenious Targeting Laboratory specializes in designing and generating custom mouse models precisely matched to your experimental requirements. Since 1998, we have completed more than 2,500 custom projects, developing expertise across every model type and therapeutic area.

Whether you need a straightforward knockout, a sophisticated conditional allele, a precise point mutation, or a complex humanization, Ingenious Targeting Laboratory's scientific team provides the design expertise and technical capability to deliver study ready animals.

## Our Custom Approach

### Scientific Consultation

Every project begins with detailed scientific consultation. Our team works with you to:

- Understand your research goals and experimental design
- Evaluate targeting strategy options
- Recommend optimal allele configurations
- Anticipate potential challenges
- Design for downstream applications

This collaborative approach ensures your model delivers the experimental capabilities you need.

### Tailored Allele Design

We design each allele specifically for your research application:

**Knockout Alleles** optimized for complete loss of function with appropriate exon selection and reading frame disruption

**Conditional Alleles** designed with LoxP site positioning that preserves gene function until Cre mediated deletion

**Knockin Alleles** engineered for precise sequence integration with maintained reading frame and expression

**Humanized Alleles** configured for species appropriate expression of human sequences

## Project Types

### Conventional Knockout Projects

Complete elimination of target gene function from earliest developmental stages. Best when:

- Gene is not required for embryonic survival
- Complete loss of function phenotype is desired
- Research question focuses on essential gene function

### Conditional Knockout Projects

LoxP flanked alleles enabling controlled gene deletion. Best when:

- Global knockout causes embryonic lethality
- Tissue specific gene function needs investigation
- Temporal control of gene deletion is required
- Comparison of multiple Cre drivers is planned

### Knockin Projects

Precise genetic modifications at defined loci including:

- Point Mutations**: Single nucleotide changes modeling disease variants
- cDNA Knockins**: Alternative coding sequence under endogenous control
- Reporter Knockins**: Fluorescent or enzymatic reporters marking gene expression
- Tag Knockins**: Epitope tags enabling protein detection and purification
- Conditional Knockins**: Controllable expression of inserted sequences

### Humanization Projects

Replacement of mouse sequences with human:

- Full Gene Humanization**: Complete gene replacement
- Exon Humanization**: Targeted domain replacement
- Immune Checkpoint Humanization**: PD1, PDL1, CTLA4, LAG3, TIM3
- Receptor Humanization**: Drug target humanization for therapeutic testing

### Complex and Multi Component Projects

Ingenious Targeting Laboratory excels in sophisticated designs including:

- Dual recombinase systems (Cre lox plus Flp FRT, Dre Rox, etc)
- Inducible expression systems (CreERT2, Tet systems)
- Multi gene modifications
- Large insertion projects (BAC transgenics)
- Safe harbor locus targeting (Rosa26, HPRT, H11)

## Project Workflow

### Phase 1: Design 

- Scientific consultation and strategy development
- Allele design and targeting planning
- Proposal and quote development
- Project approval and initiation

### Phase 2: Targeting materials 

- Design and synthesis
- Guides and/or vectors
- Sequence verification
- Quality control testing

### Phase 3: Mouse Generation 

- Injection
- Embryo transfer
- F0 birth 

### Phase 4: Germline Transmission 

- F0 breeding
- Genotyping
- Germline transmission confirmation
- Documentation and deliverable preparation

## Deliverables

### Standard Project Deliverables

**Mice**

- Germline confirmed F1 heterozygous for targeted allele
- Health certified for standard pathogens
- Genotyping protocol

**Documentation**

- Complete project report
- Materials sequence
- Confirmation data
- Allele schematic
- Breeding recommendations

### Optional Deliverables

- Expanded breeding cohorts
- Homozygous animals
- Cryopreserved sperm or embryos
- Phenotyping services

## What Researchers Say

*“I’d like to thank the ingenious team for making this mouse for us. We are so excited! Everyone at ingenious has been wonderful to work with throughout the entire process. We will definitely be in contact the next time we need a mouse!”*

*— **Julia Maxson, PhD**, Knight Cancer Institute, Oregon Health &amp; Science University*

*“I would definitely recommend your company to other scientists for their needs of creation of transgenic mouse models.”*

*— **Frank Y. Chen, MD, PhD**, Houston Behavioral Healthcare Hospital, Houston Adult Psychiatry*

*“iTL has done a tremendous job assisting us with our projects. Not only have they provided successful mouse lines, but their project management has always been on top of things. Communication was excellent, and at all times I felt I could trust the scientists at iTL with my work.”*

*— **Thomas G.H. Diekwisch, DMD, PhD (sc), PhD (phil.)**, Texas A&amp;M University College of Dentistry*

(/testimonials)

## Selected Publications

Ingenious Targeting Laboratory’s custom projects have contributed to more than 800 peer reviewed publications including:

- Landmark discoveries in cancer biology
- Breakthrough findings in neurodegeneration
- Novel insights in metabolic disease
- Fundamental immunology advances

(/publications)

## Start Your Custom Project

Our scientific consultants are ready to discuss your research goals and recommend optimal targeting strategies.

(/request-quote)

(/request-quote)

(/model-feasibility-check)

## Frequently Asked Questions

**What types of custom projects does ITL handle?**

ITL handles all types of custom genetically engineered mouse models including knockout, knockin (point mutations, reporters, humanized), conditional knockout, transgenic models, and complex multi-allele combinations. We also provide services for colony management, cryopreservation, backcrossing, and preclinical studies.

**How does the custom project process work?**

The process includes scientific consultation, materials design and synthesis, mouse generation via injection, and germline transmission. 

**What deliverables are included with custom projects?**

Standard deliverables include germline-transmitted F1 heterozygous mice, genotyping protocols, and complete project documentation including materials information and confirmation of all project milestone steps. 

**Can ITL help with complex or unusual project requirements?**

Yes. ITL excels at complex projects including multi-allele combinations, conditional approaches, humanized models, and other custom designs. Our scientific team provides consultation to evaluate feasibility and recommend optimal strategies. Contact us to discuss your specific project requirements.

## Related Resources

- (/model-generation-timeline)
- (/knockout-strategy-guide)
- (/conditional-vs-conventional-guide)
- (/technologies)
- (/pricing-overview)